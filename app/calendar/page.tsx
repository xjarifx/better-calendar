...
export default function CalendarPage() {
  ...
  const [firstDayOfWeek] = useState(0);
...
function DayView({
  ...
  return (
    <div className="flex-1 overflow-hidden flex flex-col">
      ...
      <div
        ref={scrollRef}
        className="flex-1 overflow-y-auto border border-gray-300 dark:border-gray-600 rounded-b-lg relative"
      >
        <div className="flex" style={{ height: `${24 * rowHeight}px` }}>
          <div className="w-16 flex-shrink-0">
            {Array.from({ length: 24 }, (_, i) => (
              <div
                key={i}
                className="h-[60px] text-xs text-muted-foreground text-right pr-2 pt-1 border-b border-gray-100 dark:border-gray-800"
              >
                {i === 0
                  ? "12 AM"
                  : i < 12
...
function WeekView({
  ...
  const currentTimePosition = (() => {
    ...
    return {
      ...
      top: hours * rowHeight + (minutes / 60) * rowHeight,
    };
  })();
        ...
        <div className="flex-1 grid grid-cols-7 gap-1">
          {days.map((day) => (
            ...
            >
              ...
              <div
                className={`text-sm font-semibold ${isToday(day) ? "text-primary" : ""}`}
              >
                {format(day, "d")}
              </div>
            </div>
      ...
      {allDayEvents.length > 0 && (
        <div className="border border-gray-300 dark:border-gray-600 border-b-0 rounded-t-lg pt-1 pb-2 bg-muted/30">
          <div className="flex px-2">
            <div className="w-16 flex-shrink-0 text-xs font-medium text-muted-foreground text-right pt-1 pr-2">
              All Day
            </div>
            <div className="flex-1 grid grid-cols-7 gap-1">
              {days.map((day, idx) => {
                ...
                return (
                  <div
                    key={day.toISOString()}
                    className={`min-h-[30px] ${idx < 6 ? "border-r border-gray-100 dark:border-gray-800" : ""}`}
                  >
...
function MonthView({
  ...
  for (let i = 0; i < 7; i++) {
    const day = addDays(
      startOfWeek(new Date(), { weekStartsOn: firstDayOfWeek as any }),
      i,
    );
            ...
            >
              ...
              <div className="space-y-0.5 overflow-hidden">
                {dayEvents.slice(0, 3).map((event) => (
                  <EventCard key={event.id} event={event} ultraCompact />
                ))}
                {dayEvents.length > 3 && (
